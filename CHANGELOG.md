...
- **NVIDIA dashboard integration** (2026-09-21) — NVIDIA is now a first-class vendor
  ...
  use the same interaction contract as AMD and Intel. The runtime JSON is generated by
  `tools/build-nvidia-data.py`; missing specifications remain blank.
- **Unified product discovery suite** (2026-09-21) — AMD and Intel now share exact-SKU
  search precedence, closed-card match explanations, visible identity paths, source
  provenance, persistent four-product cross-vendor comparison, and shareable URL state
  for vendor, product line, query, filters, and core range.
- **Regression coverage for discovery workflows** — the browser suite now checks exact
  matching, collapsed results, row highlighting, URL restoration, source disclosure,
  and cross-vendor comparison. AMD data floors were raised to the current official-CSV
...
- **NVIDIA press-deck theme** (2026-09-21) — replaced the temporary neutral
  ...
  accents while green remains the primary interaction and emphasis color.
...
### Fixed
- **Consistent product titles beside vendor selection** (2026-09-21) — the compact
  header now gives the vendor selector and product-line title dedicated columns, so
  longer tabs such as Ryzen and NVIDIA Data Center no longer drop the title onto a
  separate row. Counts move below at narrower widths instead.
- **Generation collapse motion** (2026-09-21) — generation disclosures now animate
  their natural content height in 180ms. Removed the oversized `max-height` transition
  that caused a visible pause and multi-stage close on every vendor.
